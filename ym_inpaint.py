...
def main(args, img_file, mask_file, save_file):
    # Pre-process
    img_path = img_file
...
if __name__ == '__main__':
    ...
    for im in tqdm(sorted(glob.glob(os.path.join(args.img_path,'*.jpg')))[10000:15000]):
        im=im.split('/')[-1]
        img_file=os.path.join(args.img_path,im)
        im = im.replace('.jpg','.png')
        save_file=os.path.join(args.save_path,im)
        mask_file=os.path.join(args.mask_path,im)

        main(args, img_file, mask_file, save_file)
